...
export const Route = createFileRoute('/work/$slug')({
  loader: ({ params }) => {
    ...
    if (project.externalUrl) {
      throw redirect({ href: project.externalUrl })
    }
...
function ProjectCaseStudyPage() {
  const { project, caseStudy } = Route.useLoaderData()
  ...
  return (
    <main className="w-full pb-16 pt-24 md:pb-24 md:pt-32">
      ...
      <div className="mx-auto w-full max-w-[1400px] bg-[var(--color-bg-soft)] px-6 md:px-0">
        <img
          src={project.image}
          alt={`${project.name} hero`}
          className="m-0 block max-h-[min(92vh,980px)] min-h-[48vh] w-full rounded-3xl object-cover object-center align-top"
          loading="eager"
          decoding="async"
        />
      </div>
...
function CaseStudySidebar({ caseStudy }: { caseStudy: ProjectCaseStudy }) {
  return (
    <div className="relative top-0 box-border h-auto overflow-y-auto px-[var(--page-gutter)] py-12 md:sticky lg:max-w-none lg:py-20 lg:pr-10 md:h-screen">
      <dl className="m-0 space-y-6 border-0 p-0">
        <div>
          ...
          <dd className="m-0 mt-1.5 font-body text-sm font-medium text-[var(--color-text)]">
            {caseStudy.year}
          </dd>
        </div>
        {caseStudy.industry ? (
        ...
        {caseStudy.sections && caseStudy.sections.length > 0
          ...
          : (
              <div className="space-y-5">
                {(caseStudy.context ?? []).map((paragraph, i) => (
                  <p
                    key={i}
                    className="m-0 max-w-prose font-body text-sm leading-relaxed text-[var(--color-text)] md:text-[0.9375rem]"
                  >
...
function CaseStudyGallery({ caseStudy }: { caseStudy: ProjectCaseStudy }) {
  return (
    <div className="m-0 mt-12 min-w-0 p-0 lg:mt-0">
